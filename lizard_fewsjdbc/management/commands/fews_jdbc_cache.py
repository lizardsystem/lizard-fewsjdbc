...
URLs, or give None as an argument to not generate any URLs.'''
    ...
    option_list = BaseCommand.option_list + (
        make_option('--deep', '-d', dest='deep', action="store_true",
                    help='Create deep cache, besides filters also do parameters and locations.'),
        make_option('--timeout', '-t', dest='timeout',
                    help='Cache timeout in seconds. Default is 8*60*60 = 8 hours.'),
        )
    ...
    def load_parameters(self, jdbc_source, tree, url_name, default_url_name):
        ...
        for item in tree:
            if item["children"]:
                ...
                logger.debug("Getting named parameters for %s." % (item["id"],))
                if default_url_name:
                    parameters = jdbc_source.get_named_parameters(
                        item["id"], ignore_cache=True, cache_timeout=self.timeout)
                else:
                    parameters = jdbc_source.get_named_parameters(
    ...
    def handle(self, *args, **options):
        ...
        for jdbc_source in JdbcSource.objects.all():
            ...
            try:
                if default_url_name:
                    tree = jdbc_source.get_filter_tree(ignore_cache=True,
                                                       cache_timeout=self.timeout)
                else:
                    tree = jdbc_source.get_filter_tree(ignore_cache=True,
                                                       url_name=url_name,
                ...
                if options['deep']:
                    ...
                    for filter_id, param_id in params:
                        logger.debug(
                            "Getting locations: %s, %s" % (filter_id, param_id))
